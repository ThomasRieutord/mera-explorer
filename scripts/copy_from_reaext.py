...
parser.add_argument(
    "--lrootdir", help="Root directory on the local host (where the files are put)"
)
parser.add_argument("--ruser", help="User name on the remote host", default="trieutord")
parser.add_argument(
    "--streams",
    help="Streams of data to copy (ANALYSIS, FC3hr, FC33hr)",
    default="ANALYSIS",
)
parser.add_argument(
    "--rdates",
    help="Range of validity dates to transfer (ex: 1991-01_2001-04 transfers all GRIB from Jan. 1991 to Apr. 2001)",
    default="1981-01_2016-12",
)
...
req_gribnames = gribs.get_all_mera_gribnames(
    req_variables, valtimes, streams=streams, pathfromroot=False
)
heregribnames = gribs.subset_present_gribnames(req_gribnames, fsname, exclude_bz2=False)
print(
    f"Found {len(heregribnames)} GRIB files in {fsname} ({100*len(heregribnames)/len(req_gribnames)} % of all)."
)
...
end_time = time.time()
print(
    f"Total: {(end_time - start_time)/60} min elapsed ({(end_time-start_time)/len(heregribnames)} s/file)"
)
